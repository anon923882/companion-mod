...
Interactive CLI for scraping image URLs from chapters on [m.happymh.com](https://m.happymh.com/) using the jina.ai proxy for reliable retrieval. The tool preserves the gofile.io-inspired interactive framing while swapping the old flow for targeted HappyMH scraping.
...
- Extracts and lists all image sources from the page, including markdown `![]()` references returned by jina.ai.
